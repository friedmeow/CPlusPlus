...
void GALAXY_SYSTEM::update()
{
	...
	*/
	for (int i = 0; i < mNumOfObjs; i++)
	{
		...
		if (mX[i] < ((-1)*mSpaceSize / 2)) mX[i] = mSpaceSize / 2;
		if (mY[i] > (mSpaceSize / 2)) mY[i] = -mSpaceSize / 2;
		if (mY[i] < ((-1)*mSpaceSize / 2)) mY[i] = mSpaceSize / 2;
		/*
		if (mX[i] >(mSpaceSize / 2)) mX[i] -= mSpaceSize;
		if (mX[i] < ((-1)*mSpaceSize / 2)) mX[i] += mSpaceSize;
		if (mY[i] > (mSpaceSize / 2)) mY[i] -= mSpaceSize ;
		if (mY[i] < ((-1)*mSpaceSize / 2)) mY[i] += mSpaceSize ;
		*/
	}
	/*
	for (int i = 0; i < mNumOfObjs; i++)
